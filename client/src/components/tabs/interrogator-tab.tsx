...
import { MessageSquare, Send, Paperclip, Bot, Sparkles } from "lucide-react";
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="text-center py-8">
            ...
            <h2 className="text-lg font-semibold mb-1">AI Interrogator</h2>
          ...
          <div className="flex gap-3">
            ...
            <div className="bg-card border rounded-lg rounded-tl-none p-4 max-w-lg">
          ...
          <div className="pl-11 space-y-3">
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Upcoming questions:</p>
            {sampleQuestions.map((q, i) => (
              <div key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                <span className="w-5 h-5 rounded-full bg-muted flex items-center justify-center text-xs shrink-0 mt-0.5">{i + 1}</span>
                <span>{q}</span>
              </div>
            ))}
          </div>
